...
def xmind_shrink(path, PNG_Quality=10, JPEG_Quality=20, use_pngquant=True,
                 replace=True, output_path=None):
    """
    ...
    :param output_path: If replace is False, compress to the output_path (default: None).
    If None, the output path is the same as the original file path, with the suffix ".shrink.xmind" added.
    """
        ...
        if os.path.exists(os.path.join(TMP_DIR, "attachments")):
            ...
            if replace:
                ...
                if output_path is None:
                    output_path = file+".shrink.xmind"
                zipDir(TMP_DIR, output_path)
        else:
